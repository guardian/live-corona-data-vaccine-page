...
# for state in states:
#     short_cols.append(f'AIR_{state}_16_PLUS_12_PLUS_FIRST')
#     short_cols.append(f'AIR_{state}_16_PLUS_12_PLUS_SECOND')

#     short_cols.append(f'AIR_{state}_12_15_12_PLUS_FIRST')
#     short_cols.append(f'AIR_{state}_12_15_12_PLUS_SECOND')

#     short_cols.append(f'AIR_{state}_5_11_12_PLUS_FIRST')
#     short_cols.append(f'AIR_{state}_5_11_12_PLUS_SECOND')


newData = pd.DataFrame(columns=['DATE_AS_AT','STATE','12_PLUS_FIRST','12_PLUS_SECOND',
 '5_11_FIRST', '5_11_SECOND', '16_PLUS_THIRD_DOSE'])
...
for state in states:
    ...
    if state == "AUS":
        temp.rename(columns={f'AIR_{state}_16_PLUS_THIRD_DOSE_COUNT': '16_PLUS_THIRD_DOSE'}, inplace=True) 
    else:
        temp.rename(columns={f'AIR_{state}_18_PLUS_THIRD_DOSE_COUNT': '16_PLUS_THIRD_DOSE'}, inplace=True) 

    temp = temp[['DATE_AS_AT','STATE', '12_PLUS_FIRST','12_PLUS_SECOND','5_11_FIRST', '5_11_SECOND', '16_PLUS_THIRD_DOSE']]
...
def makeProjection(state, cutoff_date):
    ...
    temp_state['12_plus_first'] = temp_state['12_PLUS_FIRST'].diff(1)
    temp_state['12_plus_second'] = temp_state['12_PLUS_SECOND'].diff(1)


    temp_state['12_plus_first_avg'] = temp_state['12_plus_first'].rolling(window=7).mean()
    temp_state['12_plus_second_avg'] = temp_state['12_plus_second'].rolling(window=7).mean()
    ...
    last_doses = temp_state['12_plus_first_avg'].iloc[-1]

    last_second_doses = temp_state['12_plus_second_avg'].iloc[-1]
	
    current_second_doses = temp_state['12_PLUS_SECOND'].iloc[-1]
    # print("current second", current_second_doses)

    current_first_doses = temp_state['12_PLUS_FIRST'].iloc[-1]

    current_date = temp_state['DATE_AS_AT'].iloc[-1]
    # print("currentdate", current_date)

    current_rolling = int(temp_state['12_plus_first_avg'].iloc[-1])
    current_rolling_sec = int(temp_state['12_plus_second_avg'].iloc[-1])

	# Handle data change in NT and ACT - just going to cut the dataset off from August

    temp_state = temp_state.loc[temp_state['DATE_AS_AT'] > "2021-08-01"]

    first_dose_eq_second = temp_state[temp_state['12_PLUS_FIRST'] >= current_second_doses]['DATE_AS_AT'].iloc[0]
	
    current_lag = (current_date - first_dose_eq_second).days + 1
    # print("current_lag", current_lag)

    ninety_target = five_plus[state] * 0.9
    eighty_target = five_plus[state] * 0.8
    seventy_target = five_plus[state] * 0.7
    ...
    if (eighty_vax_to_go < 0):
        ...
        days_to_go_80 = 0
        eighty_finish_first = temp_state[temp_state['12_PLUS_FIRST'] > eighty_target]['DATE_AS_AT'].iloc[0]
    ...
    if (seventy_vax_to_go < 0):
        ...
        days_to_go_70 = 0
        seventy_finish_first = temp_state[temp_state['12_PLUS_FIRST'] > seventy_target]['DATE_AS_AT'].iloc[0]
    else:
    ...
    if (ninety_vax_to_go < 0):
        ...
        days_to_go_90 = 0
        ninety_finish_first = temp_state[temp_state['12_PLUS_FIRST'] > ninety_target]['DATE_AS_AT'].iloc[0]
    else:
    ...
    if (seventy_vax_to_go_second < 0):
        ...
        seventy_finish_second = temp_state[temp_state['12_PLUS_SECOND'] > seventy_target]['DATE_AS_AT'].iloc[0]
    ...
    if (eighty_vax_to_go_second < 0):
        ...
        eighty_finish_second = temp_state[temp_state['12_PLUS_SECOND'] > eighty_target]['DATE_AS_AT'].iloc[0]
        days_to_second_80 = 0
        eighty_reached = True
    else:	
    ...
    if (ninety_vax_to_go_second < 0):
        ...
        ninety_finish_second = temp_state[temp_state['12_PLUS_SECOND'] > ninety_target]['DATE_AS_AT'].iloc[0]
        days_to_second_90 = 0
        ninety_reached = True
    else:	
    ...
    results = {"current_lag":current_lag, "current_rolling":current_rolling,

    "seventy_finish_first":seventy_finish_first, "seventy_finish_second":seventy_finish_second,  
    "eighty_finish_first": eighty_finish_first, "eighty_finish_second":eighty_finish_second, 
     "ninety_finish_first": ninety_finish_first, "ninety_finish_second": ninety_finish_second,
...
for state in states[:2]:
    ...
    inter = newData[newData['STATE'] == state].copy()
