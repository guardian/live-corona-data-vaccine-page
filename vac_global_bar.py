...
oz = df.loc[df['location'] == 'Australia'].copy()
...
#     inter = inter.drop_duplicates(subset=['people_fully_vaccinated_per_hundred'], keep='first')
#     latest = inter.loc[inter['people_fully_vaccinated_per_hundred'] == inter['people_fully_vaccinated_per_hundred'].max()].copy()
...
#                 "subtitle": f"""Showing percentage of the population in OECD countries that have received two doses of a Covid-19 vaccine. Date of latest data for each country in brackets. Last updated {latest_date}""",
#                 "footnote": "",
...
#     yachtCharter(template=template, dropdown=[],labels=labels,options=[{"enableShowMore":"TRUE", "autoSort":"FALSE"}], data=chartData, chartId=[{"type":"horizontalbar"}], chartName=f"oecd-covid-fully-vaccinated{testo}")
